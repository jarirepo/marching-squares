...
if (stream) {
	...
	if (!recorder) throw "Unable to record";
...
const recordBtn: HTMLButtonElement =
	document.querySelector<HTMLButtonElement>("button#record")!;
...
stopBtn.onclick = (event) => {
	if (recorder && recorder.getState() === "recording") {
		// NOTE: recorder.state says 'inctive'
		recorder.stopRecording(() => {
...
const grid = new Grid({
	...
	randomizer: (i: number, j: number) =>
		i > 0 && i < 30 && j > 0 && j < 40 ? 0 : 0,
});
...
const ms = new MarchingSquares(grid, { isoValue: 0.6 });
...
function animate(time = 0) {
	...
	ms.opts.isoValue = 0.5 * (Math.sin(((2 * Math.PI) / 1e4) * time) + 1);
	ms.update();
